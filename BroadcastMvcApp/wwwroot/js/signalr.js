(function webpackUniversalModuleDefinition(root, factory) {
  ...
  return /******/ (() => {
    ...
    })(LogLevel || (LogLevel = {})); // CONCATENATED MODULE: ./src/Loggers.ts
    ...
    NullLogger.instance = new NullLogger(); // CONCATENATED MODULE: ./src/Utils.ts
    ...
    class FetchHttpClient extends HttpClient {
      constructor(logger) {
        ...
        if (typeof fetch === "undefined" || Platform.isNode) {
          ...
          this._fetchType = requireFunc("fetch-cookie")(
            this._fetchType,
            this._jar,
          );
        } else {
      ...
      async send(request) {
        ...
        try {
          response = await this._fetchType(request.url, {
            ...
            cache: "no-cache",
            credentials:
              request.withCredentials === true ? "include" : "same-origin",
            headers: {
        ...
        return new Promise((resolve, reject) => {
          ...
          xhr.onload = () => {
            ...
            if (xhr.status >= 200 && xhr.status < 300) {
              ...
              reject(
                new HttpError(
                  xhr.response || xhr.responseText || xhr.statusText,
                  xhr.status,
                ),
              );
            }
    ...
    })(MessageType || (MessageType = {})); // CONCATENATED MODULE: ./src/Subject.ts
    ...
    class HubConnection {
      ...
      constructor(
        ...
        this.serverTimeoutInMilliseconds =
          ...
          serverTimeoutInMilliseconds !== void 0
            ? serverTimeoutInMilliseconds
            : DEFAULT_TIMEOUT_IN_MS;
        this.keepAliveIntervalInMilliseconds =
          keepAliveIntervalInMilliseconds !== null &&
          keepAliveIntervalInMilliseconds !== void 0
            ? keepAliveIntervalInMilliseconds
            : DEFAULT_PING_INTERVAL_IN_MS;
        this._statefulReconnectBufferSize =
          statefulReconnectBufferSize !== null &&
          statefulReconnectBufferSize !== void 0
            ? statefulReconnectBufferSize
            : DEFAULT_STATEFUL_RECONNECT_BUFFER_SIZE;
        this._logger = logger;
        this._protocol = protocol;
      ...
      stream(methodName, ...args) {
        ...
        promiseQueue = this._sendWithProtocol(invocationDescriptor).catch(
          (e) => {
            subject.error(e);
            delete this._callbacks[invocationDescriptor.invocationId];
          },
      ...
      invoke(methodName, ...args) {
        ...
        const p = new Promise((resolve, reject) => {
          ...
          const promiseQueue = this._sendWithProtocol(
      ...
      _processIncomingData(data) {
        ...
        if (data) {
          ...
          for (const message of messages) {
            if (
              this._messageBuffer &&
              !this._messageBuffer._shouldProcessMessage(message)
            ) {
            ...
            switch (message.type) {
              case MessageType.Invocation:
                this._invokeClientMethod(message).catch((e) => {
                  this._logger.log(
                    LogLevel.Error,
                    `Invoke client method threw error: ${getErrorString(e)}`,
                  );
                });
                break;
              case MessageType.StreamItem:
              case MessageType.Completion: {
                ...
                if (callback) {
                  ...
                  try {
                    ...
                    this._logger.log(
                      LogLevel.Error,
                      `Stream callback threw error: ${getErrorString(e)}`,
                    );
                  }
                }
                break;
      ...
      _processHandshakeResponse(data) {
        ...
        try {
          [remainingData, responseMessage] =
            this._handshakeProtocol.parseHandshakeResponse(data);
        } catch (e) {
      ...
      async _invokeClientMethod(invocationMessage) {
        ...
        if (!methods) {
          this._logger.log(
            LogLevel.Warning,
            `No client method with the name '${methodName}' found.`,
          );
          // No handlers provided by client but the server is expecting a response still, so we send an error
          if (invocationMessage.invocationId) {
            this._logger.log(
              LogLevel.Warning,
              `No result given for '${methodName}' method and invocation ID '${invocationMessage.invocationId}'.`,
            );
        ...
        for (const m of methodsCopy) {
          try {
            ...
            if (expectsResponse && res && prevRes) {
              this._logger.log(
                LogLevel.Error,
                `Multiple results provided for '${methodName}'. Sending error to server.`,
              );
              completionMessage = this._createCompletionMessage(
                invocationMessage.invocationId,
            ...
            this._logger.log(
              LogLevel.Error,
              `A callback for the method '${methodName}' threw error '${e}'.`,
            );
          }
        }
        if (completionMessage) {
          ...
          if (res) {
            this._logger.log(
              LogLevel.Error,
              `Result given for '${methodName}' method but server is not expecting a result.`,
            );
          }
        }
      }
      _connectionClosed(error) {
        this._logger.log(
          LogLevel.Debug,
          `HubConnection.connectionClosed(${error}) called while in state ${this._connectionState}.`,
        );
      ...
      async _reconnect(error) {
        ...
        while (nextRetryDelay !== null) {
          ...
          try {
            ...
            if (this._reconnectedCallbacks.length !== 0) {
              try {
                this._reconnectedCallbacks.forEach((c) =>
        ...
        this._logger.log(
          LogLevel.Information,
          `Reconnect retries have been exhausted after ${Date.now() - reconnectStartTime} ms and ${previousReconnectAttempts} failed attempts. Connection disconnecting.`,
        );
        this._completeClose();
      }
      ...
      _cancelCallbacksWithError(error) {
        ...
        Object.keys(callbacks).forEach((key) => {
          ...
          try {
            ...
            this._logger.log(
              LogLevel.Error,
              `Stream 'error' callback called with '${error}' threw error: ${getErrorString(e)}`,
            );
          }
        });
      }
            ...
            error: (err) => {
              ...
              promiseQueue = promiseQueue.then(() =>
                this._sendWithProtocol(
                  this._createCompletionMessage(streamId, message),
                ),
              );
            },
            next: (item) => {
              promiseQueue = promiseQueue.then(() =>
                this._sendWithProtocol(
                  this._createStreamItemMessage(streamId, item),
                ),
              );
            },
          });
    ...
    HeaderNames.Cookie = "Cookie"; // CONCATENATED MODULE: ./src/AccessTokenHttpClient.ts
    ...
    class AccessTokenHttpClient extends HttpClient {
      ...
      async send(request) {
        ...
        if (
          ...
          (!this._accessToken ||
            (request.url && request.url.indexOf("/negotiate?") > 0))
        ) {
    ...
    })(TransferFormat || (TransferFormat = {})); // CONCATENATED MODULE: ./src/AbortController.ts
            ...
            try {
              ...
              if (response.statusCode === 204) {
                ...
                if (response.content) {
                  this._logger.log(
                    LogLevel.Trace,
                    `(LongPolling transport) data received. ${getDataDetail(response.content, this._options.logMessageContent)}.`,
                  );
                  if (this.onreceive) {
            ...
            eventSource.onmessage = (e) => {
              if (this.onreceive) {
                try {
                  this._logger.log(
                    LogLevel.Trace,
                    `(SSE transport) data received. ${getDataDetail(e.data, this._options.logMessageContent)}.`,
                  );
    ...
    class HttpConnection {
      constructor(url, options = {}) {
        ...
        options.logMessageContent =
          options.logMessageContent === undefined
            ? false
            : options.logMessageContent;
        if (
        ...
        options.timeout =
          options.timeout === undefined ? 100 * 1000 : options.timeout;
        ...
        this._httpClient = new AccessTokenHttpClient(
          options.httpClient || new DefaultHttpClient(this._logger),
      ...
      async _getNegotiationResponse(url) {
        ...
        try {
          ...
          if (
            negotiateResponse.useStatefulReconnect &&
          ...
          let errorMessage =
            "Failed to complete negotiation with the server: " + e;
          ...
          return Promise.reject(
            new FailedToNegotiateWithServerError(errorMessage),
          );
        }
      }
      _createConnectUrl(url, connectionToken) {
        ...
        return (
          url + (url.indexOf("?") === -1 ? "?" : "&") + `id=${connectionToken}`
        );
      }
      async _createTransport(
        ...
        for (const endpoint of transports) {
          const transportOrError = this._resolveTransportOrError(
            ...
            requestedTransport,
            requestedTransferFormat,
            (negotiate === null || negotiate === void 0
              ? void 0
          ...
          if (transportOrError instanceof Error) {
            ...
            if (!negotiate) {
              ...
              connectUrl = this._createConnectUrl(
                url,
      ...
      _stopConnection(error) {
        this._logger.log(
          LogLevel.Debug,
          `HttpConnection.stopConnection(${error}) called while in state ${this._connectionState}.`,
        );
        this.transport = undefined;
      ...
      _resolveNegotiateUrl(url) {
        ...
        if (!searchParams.has("negotiateVersion")) {
          searchParams.append(
            "negotiateVersion",
            this._negotiateVersion.toString(),
          );
        }
